...
- `ix`の存在確認と`.agents`列挙を同一コマンドで実行した際、`.agents`列挙側が終了コード1となった。ソースへの影響はなく、以後は存在確認とIxコマンドを個別に実行する。
- Ixのグラフ更新を実行したところ、Ix CLIが依存する`node`がこのPowerShellセッションのPATHにないため開始できなかった。ソースへの影響はなく、構造確認は対象ファイルを直接読み取り、検証はDenoで続行する。
- CSS検索のツール呼び出しで名前空間を誤指定し、検索を開始できなかった。ソースへの影響はなく、正しいシェル呼び出しで同じ検索を再実行する。
- 支出編集レイアウト修正後にIxの再マップを試みたが、依存する`node`がPATHにないため実行できなかった。ソースへの影響はなく、Denoによる型検査とビルドで修正を確認する。
