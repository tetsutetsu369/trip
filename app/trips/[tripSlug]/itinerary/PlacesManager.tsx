...
export default function PlacesManager({ tripId }: { tripId: string }) {
  ...
  const save = async (event: FormEvent) => {
    ...
    if (!name) { setStatus("行き先名を入力してください"); return; }
    ...
    try {
      ...
      setEditing(null); setDraft(empty); await load(currentEditing ? "行き先を更新しました" : "行き先を追加しました");
    } catch {
