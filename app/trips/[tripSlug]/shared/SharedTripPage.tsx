...
export default function SharedTripPage({
  tripId, tripSlug, tripDates, participants, isAdmin = false, focus = "all",
}: { tripId: string; tripSlug: string; tripDates: { start: string; end: string }; participants: Person[]; isAdmin?: boolean; focus?: Focus }) {
  ...
  return <main className="shared-shell"><TripHeader tripSlug={tripSlug} active={focus === "all" ? "portal" : focus} showAdmin={isAdmin} /><p className="shared-kicker">SHARED TRIP DATA</p><h1>{focus === "itinerary" ? "旅程を管理" : focus === "packing" ? "持ち物を管理" : focus === "notes" ? "共有メモ" : "旅の共有データ"}</h1><p className="shared-status" role="status">{status}</p>
