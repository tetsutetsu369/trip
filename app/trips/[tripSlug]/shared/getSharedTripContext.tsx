...
export async function getSharedTripContext(tripSlug: string, nextPath: string) {
  ...
  try {
    ...
    const { data: participants, error: participantsError } = await supabase.from("trip_participants").select("id,display_name,avatar_color").eq("trip_id", tripId).order("created_at");
    if (participantsError) throw participantsError;
    const site = getTripSiteConfig(tripSlug);
    if (!site) throw new Error("Trip site configuration not found");
    return { tripId, tripSlug, userId: userData.user.id, tripDates: { start: site.startDate, end: site.endDate }, participants: participants ?? [], isAdmin: membership.role === "admin" };
  } catch (error) {
