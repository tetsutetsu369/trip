...
const allocationLabel: Record<AllocationMethod, string> = { equal_all: "全員で均等", equal_selected: "対象者で均等", custom: "負担額を指定", personal: "個人負担" };
...
export default function FinancePage({ tripId, tripSlug, tripName, avatarUrl = null, userId }: { tripId: string; tripSlug: string; tripName: string; avatarUrl?: string | null; userId: string }) {
  ...
  const load = async (message = "みんなに共有済み") => {
    ...
    if (!receiptShareIds.length) setReceiptShareIds(userIds);
    setStatus(message);
  };
  ...
  const balances = useMemo(() => {
    ...
    for (const expense of expenses) {
      ...
      for (const share of shares.filter((item) => item.expense_id === expense.id)) result.set(share.user_id, (result.get(share.user_id) ?? 0) - amountOf(share.amount));
    }
  ...
  const saveBudget = async (event: FormEvent) => {
    ...
    setSaving(true);
    const nextBudget = { ...budgetSettings, budgetPerPerson: amountOf(budgetPerPerson), budgetParticipantIds, participantBudgets, categoryBudgets, fuelPrice: amountOf(fuelPrice), fuelEfficiency: Math.max(1, decimalOf(fuelEfficiency)) };
  ...
  const newExpenseDraft = (): ExpenseDraft => ({ id: null, version: null, receipt_id: null, purchase_id: null, title: "", category: "food", planned_amount: 0, actual_amount: 0, payment_status: "unpaid", payer_id: userId, itinerary_item_id: "", allocation_method: "equal_all", share_user_ids: people.map((person) => person.id), share_amounts: splitEvenly(0, people.map((person) => person.id)), memo: "" });
  ...
  const saveExpense = async (event: FormEvent) => {
    ...
    const shareUserIds = expenseDraft.allocation_method === "equal_all" ? people.map((person) => person.id) : expenseDraft.allocation_method === "personal" ? (expenseDraft.payer_id ? [expenseDraft.payer_id] : []) : expenseDraft.share_user_ids;
    ...
    setSaving(true);
    const { data, error } = await supabase.rpc("save_expense_with_shares", { target_trip_id: tripId, target_expense_id: expenseDraft.id, expected_version: expenseDraft.version, input_title: expenseDraft.title.trim(), input_category: expenseDraft.category, input_planned_amount: amountOf(expenseDraft.planned_amount), input_actual_amount: amountOf(expenseDraft.actual_amount), input_payment_status: expenseDraft.payment_status, input_payer_id: expenseDraft.payer_id || null, input_itinerary_item_id: expenseDraft.itinerary_item_id || null, input_purchase_id: expenseDraft.purchase_id, input_receipt_id: expenseDraft.receipt_id, input_allocation_method: expenseDraft.allocation_method, input_share_user_ids: shareUserIds, input_share_amounts: shareAmounts, input_memo: expenseDraft.memo.trim() });
    setSaving(false);
  ...
  const submitReceipt = async (event: FormEvent) => {
    event.preventDefault();
    if (!supabase || !receiptDraft.store_name.trim() || !items.some((item) => item.name.trim() && item.net_amount > 0) || !receiptShareIds.length) { setStatus("店名・明細・負担者を入力してください"); return; }
    setSaving(true);
    ...
    const { data, error } = await supabase.rpc("create_receipt_expense_with_shares", { target_trip_id: tripId, input_store_name: receiptDraft.store_name.trim(), input_purchased_on: receiptDraft.purchased_on || null, input_payer_id: receiptDraft.payer_id || null, input_category: receiptDraft.category, input_memo: receiptDraft.memo.trim(), input_items: inputItems, input_itinerary_item_id: receiptDraft.itinerary_item_id || null, input_allocation_method: "equal_selected", input_share_user_ids: receiptShareIds, input_share_amounts: {} });
    setSaving(false);
    if (error || data?.status !== "ok") { setStatus("レシートを保存できませんでした"); return; }
    setReceiptDraft({ store_name: "", purchased_on: today(), payer_id: userId, category: "receipt", itinerary_item_id: "", memo: "" }); setItems([{ name: "", category: "food", net_amount: 0, tax_rate: 0.1 }]); await load("レシートと費用を保存しました");
  };
  ...
  return <main className="budget-shell finance-shell"><TripHeader tripSlug={tripSlug} tripName={tripName} avatarUrl={avatarUrl} /><p className="save-status" role="status">{status}</p>
    ...
    <section className="panel budget-planner"><div className="budget-planner-heading"><div><h3>1人あたりの予算と項目別配分</h3><p>参加者ごとの予算を合計し、登録した費用の予定額・実績額を同じ予算から差し引きます。</p></div><form className="budget-planner-form unified-budget-form" onSubmit={saveBudget}><label><span>基本の1人あたり予算</span><input type="number" min="0" step="100" value={budgetPerPerson} onChange={(event) => { setBudgetPerPerson(amountOf(event.target.value)); setBudgetDirty(true); }} disabled={!budgetReady || saving} /></label><label><span>ガソリン単価（円/L）</span><input type="number" min="0" step="1" value={fuelPrice} onChange={(event) => { setFuelPrice(amountOf(event.target.value)); setBudgetDirty(true); }} disabled={!budgetReady || saving} /></label><label><span>実燃費（km/L）</span><input type="number" min="1" step="0.1" value={fuelEfficiency} onChange={(event) => { setFuelEfficiency(Math.max(1, decimalOf(event.target.value))); setBudgetDirty(true); }} disabled={!budgetReady || saving} /></label><button className="save-button" disabled={!budgetReady || saving}>{saving ? "保存中…" : "予算を保存"}</button></form></div>
      <details className="budget-participant-settings" open><summary>予算に含める参加者と個別調整（{budgetParticipantIds.length}/{budgetPeople.length}人）</summary><div className="budget-participant-list"><div className="budget-participant-heading"><strong>予算対象の参加者</strong><span>個別予算を空欄にすると基本予算を使用</span></div>{budgetPeople.map((person) => <div className="budget-participant-row" key={person.id}><label className="budget-participant-check"><input type="checkbox" checked={budgetParticipantIds.includes(person.id)} onChange={() => toggleBudgetParticipant(person.id)} disabled={saving} /><span><strong>{person.name}</strong><small>{person.profile_id ? "ログイン済み" : "仮登録"}</small></span></label><label className="budget-participant-amount"><span>個別予算</span><input type="number" min="0" step="100" value={participantBudgets[person.id] ?? ""} placeholder={money(budgetPerPerson)} onChange={(event) => setParticipantBudget(person.id, event.target.value)} disabled={!budgetParticipantIds.includes(person.id) || saving} /></label></div>)}{!budgetPeople.length && <p className="empty-state">参加者を登録すると予算対象を選べます。</p>}</div></details>
      <div className="category-budget-grid">{categories.map((category) => <label key={category}><span>{categoryLabel[category]}の配分予算</span><input type="number" min="0" step="100" value={categoryBudgets[category] ?? ""} placeholder="未設定" onChange={(event) => setCategoryBudget(category, event.target.value)} disabled={saving} /><small>見込み {money(budgetTotals.byCategory.find((item) => item.category === category)?.planned ?? 0)}</small></label>)}</div>
