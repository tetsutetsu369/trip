...
const participantIdOf = (participants: BudgetPerson[], profileId: string | null) => participants.find((participant) => participant.profile_id === profileId)?.id ?? null;
...
export default function FinancePage({ tripId, tripSlug, tripName, avatarUrl = null, userId }: { tripId: string; tripSlug: string; tripName: string; avatarUrl?: string | null; userId: string }) {
  ...
  const nameOf = (id: string | null) => budgetPeople.find((person) => person.id === id)?.name ?? people.find((person) => person.id === id)?.name ?? "未設定";
  ...
  const load = async (message = "みんなに共有済み") => {
    ...
    const [settingsResult, receiptResult, itemResult, expenseResult, shareResult, settlementResult, memberResult, profileMemberResult, itineraryResult] = await Promise.all([
      ...
      supabase.from("receipts").select("id,store_name,purchased_on,payer_id,payer_participant_id,memo,version").eq("trip_id", tripId).order("purchased_on", { ascending: false }),
      supabase.from("receipt_items").select("id,receipt_id,name,category,net_amount,tax_rate,tax_amount,gross_amount"),
      supabase.from("expenses").select("id,receipt_id,purchase_id,itinerary_item_id,title,category,planned_amount,amount,payer_id,payer_participant_id,payment_status,settlement_status,allocation_method,memo,version").eq("trip_id", tripId).order("created_at", { ascending: false }),
      supabase.from("expense_shares").select("expense_id,user_id,participant_id,amount"),
      supabase.from("settlements").select("id,from_user_id,to_user_id,from_participant_id,to_participant_id,amount,status").eq("trip_id", tripId).order("created_at", { ascending: false }),
    ...
    const participantIdFor = (profileId: string | null) => participantIdOf(savedBudgetPeople, profileId);
    const savedSettlementParticipantIds = Array.isArray(savedBudget.settlementParticipantIds)
      ? savedBudget.settlementParticipantIds.map((id) => typeof id === "string" ? savedBudgetPeople.find((person) => person.id === id || person.profile_id === id)?.id : null).filter((id): id is string => Boolean(id))
      : savedBudgetPeople.map((person) => person.id);
    const savedParticipantIds = Array.isArray(savedBudget.budgetParticipantIds)
      ? savedBudget.budgetParticipantIds.map((id) => typeof id === "string" ? savedBudgetPeople.find((person) => person.id === id || person.profile_id === id)?.id : null).filter((id): id is string => Boolean(id))
      : savedBudgetPeople.map((person) => person.id);
    ...
    setReceipts((receiptResult.data ?? []).map((receipt) => ({ ...receipt, payer_id: receipt.payer_participant_id ?? participantIdFor(receipt.payer_id), items: itemMap.get(receipt.id) ?? [] })));
    setExpenses((expenseResult.data ?? []).map((expense) => ({ ...expense, payer_id: expense.payer_participant_id ?? participantIdFor(expense.payer_id), allocation_method: (expense.allocation_method || "equal_all") as AllocationMethod })));
    setShares((shareResult.data ?? []).flatMap((share) => {
      const participantId = share.participant_id ?? participantIdFor(share.user_id);
      return participantId ? [{ expense_id: share.expense_id, user_id: participantId, amount: share.amount }] : [];
    }));
    setSettlements((settlementResult.data ?? []).flatMap((settlement) => {
      const fromId = settlement.from_participant_id ?? participantIdFor(settlement.from_user_id);
      const toId = settlement.to_participant_id ?? participantIdFor(settlement.to_user_id);
      return fromId && toId ? [{ id: settlement.id, from_user_id: fromId, to_user_id: toId, amount: settlement.amount, status: settlement.status }] : [];
    }));
    ...
    setReceiptDraft((current) => ({ ...current, payer_id: current.payer_id === userId || !current.payer_id ? participantIdFor(userId) ?? "" : current.payer_id }));
    setStatus(message);
  };
  ...
  }, [budgetPeople, settlementParticipantIds]);
  ...
  const newExpenseDraft = (): ExpenseDraft => { const currentParticipantId = participantIdOf(budgetPeople, userId) ?? ""; return { id: null, version: null, receipt_id: null, purchase_id: null, title: "", category: "food", planned_amount: 0, actual_amount: 0, payment_status: "unpaid", payer_id: settlementParticipantIds.includes(currentParticipantId) ? currentParticipantId : "", itinerary_item_id: "", allocation_method: "equal_all", share_user_ids: settlementPeople.map((person) => person.id), share_amounts: splitEvenly(0, settlementPeople.map((person) => person.id)), memo: "" }; };
  ...
  const saveExpense = async (event: FormEvent) => {
    ...
    const shareAmounts = expenseDraft.allocation_method === "custom" ? Object.fromEntries(Object.entries(expenseDraft.share_amounts).filter(([id, amount]) => selected.has(id) && amountOf(amount) > 0).map(([id, amount]) => [id, amountOf(amount)])) : {};
  ...
  const submitReceipt = async (event: FormEvent) => {
    ...
    const currentParticipantId = participantIdOf(budgetPeople, userId) ?? "";
    setReceiptDraft({ store_name: "", purchased_on: today(), payer_id: settlementParticipantIds.includes(currentParticipantId) ? currentParticipantId : "", category: "receipt", itinerary_item_id: "", memo: "" }); setItems([{ name: "", category: "food", net_amount: 0, tax_rate: 0.1 }]); await load("レシートと費用を保存しました");
  };
  ...
  return <main className="budget-shell finance-shell"><TripHeader tripSlug={tripSlug} tripName={tripName} avatarUrl={avatarUrl} /><p className="save-status" role="status">{status}</p>
    ...
    <section className="summary-grid finance-summary"><Summary label="1人あたり予算" value={budgetTotals.participantCount ? budgetTotals.total / budgetTotals.participantCount : 0} /><Summary label="予定総額" value={totals.forecast} /><Summary label="支払済み・立替" value={totals.actual} tone="paid" /><Summary label={budgetTotals.remaining < 0 ? "予算オーバー" : "予算残額"} value={Math.abs(budgetTotals.remaining)} tone={budgetTotals.remaining < 0 ? "due" : ""} /><Summary label="未払い予定" value={totals.plannedOnly} /><Summary label="精算候補" value={suggestions.reduce((sum, item) => sum + item.amount, 0)} tone="due" /></section>
     <p className="finance-summary-note">予定総額は支払済み実績・未払い予定・行程の移動費を合計します。精算候補は支払済みで支払者・負担者を設定した費用だけです。</p>
