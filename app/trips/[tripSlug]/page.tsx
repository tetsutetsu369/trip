...
export default async function TripPortalPage({ params }: { params: Promise<{ tripSlug: string }> }) {
  ...
  if (supabase) {
    ...
    try {
      ...
      const { data: membership, error: membershipError } = await supabase.from("trip_members").select("status").eq("trip_id", tripId).eq("user_id", userData.user.id).maybeSingle<{ status: string }>();
  ...
  return <main className={`trip-portal-shell ${site.theme.heroClassName}`} style={{ "--trip-accent": site.theme.accent, "--trip-accent-dark": site.theme.accentDark, "--trip-surface": site.theme.surface, "--trip-background": site.theme.background } as CSSProperties}>
    <section className="trip-portal-hero"><p className="trip-portal-eyebrow">{site.eyebrow}</p><h1>{site.title}</h1><p className="trip-portal-description">{site.description}</p><div className="trip-portal-meta"><span>{site.dateLabel}</span><span>{site.locationLabel}</span></div></section>
    <section className="trip-portal-section" aria-labelledby="trip-overview"><div className="trip-section-heading"><div><p>TRIP OVERVIEW</p><h2 id="trip-overview">旅のいま</h2></div><span>SHARED SPACE</span></div><div className="portal-preview-grid">
      <PreviewCard href={`/trips/${site.slug}/shared#itinerary`} title="次の旅程" meta="TIMELINE">{preview.itinerary.length ? <div className="portal-mini-timeline">{preview.itinerary.map((item, index) => <div key={`${item.title}-${index}`}><b>{item.event_time?.slice(0, 5) || "未定"}</b><span>{item.title}</span><small>{item.place}</small></div>)}</div> : <p>まだ予定はありません</p>}</PreviewCard>
      ...
      <PreviewCard href={`/trips/${site.slug}/shared#packing`} title="持ち物" meta="PACKING">{preview.packing.length ? <ul className="portal-check-list">{preview.packing.map((item) => <li key={item.name}>{item.is_ready ? "✓" : "○"} {item.name}</li>)}</ul> : <p>まだ持ち物はありません</p>}</PreviewCard>
      <PreviewCard href={`/trips/${site.slug}/shared#notes`} title="共有メモ" meta="NOTES">{preview.note ? <><strong>{preview.note.title}</strong><p>{preview.note.body || "本文はまだありません"}</p></> : <p>まだ共有メモはありません</p>}</PreviewCard>
    </div></section>
