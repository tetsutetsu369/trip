...
export default async function ProfilePage({ params }: { params: Promise<{ tripSlug: string }> }) {
  ...
  return <main className="profile-shell"><TripHeader tripSlug={tripSlug} active="profile" showAdmin={context.isAdmin} /><section className="profile-card"><p className="auth-eyebrow">YOUR PROFILE</p><h1>プロフィールを編集</h1><p className="profile-intro">旅の仲間に表示される名前と自己紹介を設定できます。</p><ProfileEditor userId={context.userId} profile={profile ?? { nickname: "", line_display_name: "", avatar_url: null, avatar_color: "#e2793f", bio: "" }} /></section></main>;
}
