...
export default function ProfileEditor({ userId, profile }: { userId: string; profile: Profile }) {
  ...
  return <form className="profile-form" onSubmit={save}><div className="profile-avatar" style={{ background: value.avatar_color || "#e2793f" }}>{value.nickname.slice(0, 1) || "?"}</div><label>表示名<input required value={value.nickname} onChange={(e) => setValue({ ...value, nickname: e.target.value })} /></label><label>LINE表示名<span className="readonly-value">{value.line_display_name || "未設定"}</span></label><label>自己紹介<textarea value={value.bio} onChange={(e) => setValue({ ...value, bio: e.target.value })} placeholder="旅の仲間へのひとこと" /></label><label>アイコン色<input type="color" value={value.avatar_color || "#e2793f"} onChange={(e) => setValue({ ...value, avatar_color: e.target.value })} /></label><button className="save-button" disabled={saving}>{saving ? "保存中…" : "プロフィールを保存"}</button>{status && <p className="profile-status" role="status">{status}</p>}</form>;
}
