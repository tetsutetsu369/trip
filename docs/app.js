...
async function render() {
  try {
    ...
    root.innerHTML = `<main class="portal-shell"><section class="portal-hero"><p class="portal-kicker">TRIP PORTAL</p><h1>${escapeHtml(trip.name)}</h1><p>${escapeHtml(trip.description)}</p><div class="portal-meta"><span>${escapeHtml(trip.start_date)} 〜 ${escapeHtml(trip.end_date)}</span></div><div class="portal-hero-actions"><a class="portal-action primary" href="./shikoku-saburo-bbq-2026/shared/">旅程の詳細</a><a class="portal-action" href="./shikoku-saburo-bbq-2026/">費用の詳細</a><button class="portal-action" id="logout">ログアウト</button></div></section><section class="portal-section"><div class="section-heading"><div><p>ITINERARY</p><h2>旅程</h2></div><a class="section-link" href="./shikoku-saburo-bbq-2026/shared/">詳細を見る →</a><span>${(items.data || []).length} 件の予定</span></div><div class="itinerary-list">${itinerary}</div></section><section class="portal-section"><div class="portal-summary-grid"><article class="portal-summary-card budget"><p>現在の概算合計</p><strong>¥${total.toLocaleString()}</strong><small>1人あたり ¥${perPerson.toLocaleString()}</small><a class="summary-link" href="./shikoku-saburo-bbq-2026/">費用の詳細を見る →</a></article><article class="portal-summary-card packing"><p>持ち物</p><strong>${(packing.data || []).filter((item) => item.is_ready).length}<small> / ${(packing.data || []).length} 準備済み</small></strong><ul>${packingList}</ul></article></div></section><section class="portal-section"><div class="section-heading"><div><p>SHARED NOTES</p><h2>共有メモ</h2></div><a class="section-link" href="./shikoku-saburo-bbq-2026/shared/#notes">詳細を見る →</a></div><div class="notes-grid">${notesList}</div></section></main>`;
    bindActions();
  } catch (error) {
    console.error(error);
