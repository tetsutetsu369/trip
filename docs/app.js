...
if (!config) { root.innerHTML = `<main class="portal-shell"><section class="portal-hero"><h1>公開設定を読み込めませんでした</h1></section></main>`; throw new Error("Missing public configuration"); }
...
const header = (role = "member", avatarUrl = null) => `<header class="trip-header"><a class="trip-header-brand" href="${base}/"><span class="trip-header-mark">TJ</span><span><strong>Trip Journal</strong><small>四国三郎の郷 BBQ旅</small></span></a><nav class="trip-header-nav"><a class="is-active" href="${base}/">ポータル</a><a href="${link("itinerary/")}">旅程</a><a href="${link()}">費用</a><a href="${link("packing/")}">持ち物</a><a href="${link("notes/")}">メモ</a>${role === "admin" ? `<a class="admin-link" href="${link("admin/")}">管理</a>` : ""}<a class="trip-header-user" href="${link("profile/")}" aria-label="プロフィールを編集" title="プロフィールを編集">${avatarUrl ? `<img src="${esc(avatarUrl)}" alt="">` : "<span>👤</span>"}</a></nav></header>`;
...
async function render() {
  ...
  root.innerHTML = `<main class="portal-shell">${header(membership.role, profile?.avatar_url)}<section class="portal-hero"><p class="portal-kicker">TRIP PORTAL</p><h1>${esc(trip.name)}</h1><p>${esc(trip.description || "旅の共有ポータル")}</p><div class="portal-meta"><span>${esc(trip.start_date)} — ${esc(trip.end_date)}</span></div></section><section class="portal-section"><div class="section-heading"><div><p>ITINERARY</p><h2>旅程</h2></div></div><div class="portal-preview">${itinerary}<a class="portal-detail-link" href="${link("itinerary/")}">旅程の詳細を見る →</a></div></section><section class="portal-section"><div class="portal-summary-grid"><div class="portal-summary-card budget"><p>費用計算</p><strong>¥${total.toLocaleString()}</strong><small>1人あたり ¥${perPerson.toLocaleString()}</small><a class="portal-detail-link" href="${link()}">費用計算の詳細を見る →</a></div><div class="portal-summary-card packing"><p>持ち物</p><strong>${(packing.data || []).filter((item) => item.is_ready).length}<small> / ${(packing.data || []).length} 準備済み</small></strong><ul>${packingList}</ul><a class="portal-detail-link" href="${link("packing/")}">持ち物の詳細を見る →</a></div></div></section><section class="portal-section"><div class="section-heading"><div><p>SHARED NOTES</p><h2>共有メモ</h2></div></div><div class="note-card">${note ? `<p>${esc(note.title)}</p><span>${esc(note.body || "")}</span>` : `<span>まだ共有メモはありません。</span>`}<a class="portal-detail-link" href="${link("notes/")}">メモの詳細を見る →</a></div></section></main>`;
}

function pending() { root.innerHTML = `<main class="portal-shell"><section class="portal-hero"><h1>参加承認を確認中です</h1><p>管理者の承認が完了すると、旅のデータを表示できます。</p><button class="portal-action" id="logout">ログアウト</button></section></main>`; document.querySelector("#logout").onclick = logout; }
