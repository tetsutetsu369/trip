...
async function render() {
  ...
  root.innerHTML = `<main class="portal-shell">${header(membership.role, profile?.avatar_url)}<section class="portal-hero"><p class="portal-kicker">TRIP PORTAL</p><h1>${esc(trip.name)}</h1><p>${esc(trip.description || "旅の共有ポータル")}</p><div class="portal-meta"><span>${esc(trip.start_date)} — ${esc(trip.end_date)}</span></div></section><section class="portal-section"><div class="section-heading"><div><p>ITINERARY</p><h2>旅程</h2></div></div><a class="portal-route" href="${link("itinerary/")}">${itinerary}</a></section><section class="portal-section"><div class="portal-summary-grid"><a class="portal-summary-card budget portal-route" href="${link()}"><p>費用計算</p><strong>¥${total.toLocaleString()}</strong><small>1人あたり ¥${perPerson.toLocaleString()}</small></a><a class="portal-summary-card packing portal-route" href="${link("packing/")}"><p>持ち物</p><strong>${(packing.data || []).filter((item) => item.is_ready).length}<small> / ${(packing.data || []).length} 準備済み</small></strong><ul>${packingList}</ul></a></div></section><section class="portal-section"><div class="section-heading"><div><p>SHARED NOTES</p><h2>共有メモ</h2></div></div><a class="portal-route note-card" href="${link("notes/")}">${note ? `<p>${esc(note.title)}</p><span>${esc(note.body || "")}</span>` : `<span>まだ共有メモはありません。</span>`}</a></section></main>`;
}
