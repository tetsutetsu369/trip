...
function NumberField({label,value,set,min=0}:{label:string;value:number;set:(n:number)=>void;min?:number}){const id=useId();return <div className="field"><label htmlFor={id}>{label}</label><input id={id} type="number" min={min} value={value || ""} placeholder="0" onChange={e=>set(e.target.value === "" ? 0 : Math.max(min,positive(e.target.value)))}/></div>}
