...
export default function BudgetPage(){
  ...
  useEffect(()=>{try{const saved=localStorage.getItem(STORAGE_KEY);if(saved)setPlan({...initialPlan,...JSON.parse(saved)});}catch{}setReady(true);},[]);
  useEffect(()=>{if(!ready)return;localStorage.setItem(STORAGE_KEY,JSON.stringify(plan));setSavedAt(new Date().toLocaleTimeString("ja-JP",{hour:"2-digit",minute:"2-digit"}));},[plan,ready]);
  ...
  const update=<K extends keyof Plan>(key:K,value:Plan[K])=>setPlan(v=>({...v,[key]:value}));
  ...
  return <main className="budget-shell">
    ...
    <div className="save-toolbar"><div><strong>この端末に自動保存</strong><span>{savedAt?"最終保存 "+savedAt:"保存を準備中…"}</span></div><div className="save-actions"><button onClick={exportBackup}>バックアップ</button><button onClick={()=>fileInput.current?.click()}>復元</button><button className="danger" onClick={reset}>初期化</button></div><input ref={fileInput} hidden type="file" accept="application/json" onChange={importBackup}/></div>
    ...
    <section className="panel purchase-panel">
      ...
      <p className="note">金額は予定額でもOK。購入したら「済」にチェックすると支払済みへ移動する。未割当の予算は {money(totals.unallocated)}。</p>
      ...
      <button className="add-button" onClick={addPurchase}>＋ 買うものを追加</button><Header/>{pair("購入品の予定額",totals.purchaseTotal)}{pair("購入済み",totals.purchasePaid)}{pair("未購入＋未割当",totals.purchaseDue)}
    </section>
